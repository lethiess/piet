...
{
    [Fact]
    public void GetColorCommands_ValidInput_ColorLightCyan_MustReturn_NotNull()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.LightCyan);
        Assert.NotNull(colorCommands);
    }

    [Fact]
    public void GetColorCommands_ValidInput_ColorLightCyan_MustReturn_MustMatch()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.LightCyan);
        Assert.NotNull(colorCommands);
        ...
        Assert.Equal(Command.Add, colorCommands[0, 4].Command);
        Assert.Equal(Command.Divide, colorCommands[0, 5].Command);

        Assert.Equal(Command.Pointer, colorCommands[1, 0].Command);
        Assert.Equal(Command.Roll, colorCommands[1, 1].Command);
        Assert.Equal(Command.OutputNumber, colorCommands[1, 2].Command);
        Assert.Equal(Command.Push, colorCommands[1, 3].Command);
        Assert.Equal(Command.Subtract, colorCommands[1, 4].Command);
        Assert.Equal(Command.Modulo, colorCommands[1, 5].Command);

        Assert.Equal(Command.Switch, colorCommands[2, 0].Command);
        Assert.Equal(Command.InputNumber, colorCommands[2, 1].Command);
        Assert.Equal(Command.OutputCharacter, colorCommands[2, 2].Command);
        Assert.Equal(Command.Pop, colorCommands[2, 3].Command);
        Assert.Equal(Command.Multiply, colorCommands[2, 4].Command);
        Assert.Equal(Command.Not, colorCommands[2, 5].Command);
