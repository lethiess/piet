...
{
    [Fact]
    public void GetColorCommands_ValidInput_ColorGreen_MustReturn_NotNull()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.Green);
        Assert.NotNull(colorCommands);
    }

    [Fact]
    public void GetColorCommands_ValidInput_ColorGreen_MustReturn_MustMatch()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.Green);
        Assert.NotNull(colorCommands);

        // validate commands
        Assert.Equal(Command.InputNumber, colorCommands[0, 0].Command);
        Assert.Equal(Command.OutputCharacter, colorCommands[0, 1].Command);
        Assert.Equal(Command.Pop, colorCommands[0, 2].Command);
        Assert.Equal(Command.Multiply, colorCommands[0, 3].Command);
        Assert.Equal(Command.Not, colorCommands[0, 4].Command);
        Assert.Equal(Command.Switch, colorCommands[0, 5].Command);

        Assert.Equal(Command.Duplicate, colorCommands[1, 0].Command);
        Assert.Equal(Command.InputCharacter, colorCommands[1, 1].Command);
        ...
        Assert.Equal(Command.Push, colorCommands[2, 2].Command);
        Assert.Equal(Command.Subtract, colorCommands[2, 3].Command);
        Assert.Equal(Command.Modulo, colorCommands[2, 4].Command);
        Assert.Equal(Command.Pointer, colorCommands[2, 5].Command);
