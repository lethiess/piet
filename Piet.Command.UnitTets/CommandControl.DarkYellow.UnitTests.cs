...
{
    [Fact]
    public void GetColorCommands_ValidInput_ColorDarkYellow_MustReturn_NotNull()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.DarkYellow);
        Assert.NotNull(colorCommands);
    }

    [Fact]
    public void GetColorCommands_ValidInput_ColorDarkYellow_MustReturn_MustMatch()
    {
        var colorCommands = PietColorCommandControl.GetColorCommands(PietColors.DarkYellow);
        Assert.NotNull(colorCommands);

        // validate commands
        Assert.Equal(Command.OutputNumber, colorCommands[0, 0].Command);
        Assert.Equal(Command.Push, colorCommands[0, 1].Command);
        Assert.Equal(Command.Subtract, colorCommands[0, 2].Command);
        Assert.Equal(Command.Modulo, colorCommands[0, 3].Command);
        Assert.Equal(Command.Pointer, colorCommands[0, 4].Command);
        Assert.Equal(Command.Roll, colorCommands[0, 5].Command);

        Assert.Equal(Command.OutputCharacter, colorCommands[1, 0].Command);
        Assert.Equal(Command.Pop, colorCommands[1, 1].Command);
        Assert.Equal(Command.Multiply, colorCommands[1, 2].Command);
        Assert.Equal(Command.Not, colorCommands[1, 3].Command);
        Assert.Equal(Command.Switch, colorCommands[1, 4].Command);
        Assert.Equal(Command.InputNumber, colorCommands[1, 5].Command);

        Assert.Equal(Command.InputCharacter, colorCommands[2, 0].Command);
